...
# the agents from ~/.claude/agents/). Routed through `awok deploy` (single source of
# truth) so the deploy also materializes per-invocation `effort:` into the deployed
# agent frontmatter — the Task tool has no effort argument. Additive — never deletes
# skills/agents awok does not own. Override the target with CLAUDE_HOME=...
CLAUDE_HOME="${CLAUDE_HOME:-$HOME/.claude}"
if [ -d "$HERE/src/skills" ] || [ -d "$HERE/src/agents" ]; then
  CLAUDE_HOME="$CLAUDE_HOME" "$BINDIR/awok" --workdir "$HERE" deploy
fi
