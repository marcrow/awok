...
function paramRow(p, idx, d, ctx, rerender) {
  ...
  b.appendChild(span2(flagsRow(null, [
    flagToggle("required", !!p.required, v => { p.required = v; if (v) delete p.default; rerender(); ctx.refreshView(); },
      { title: "required and default are mutually exclusive" }),
  ])));
...
function outputRow(m, d, o, idx, ctx, rerender) {
  ...
  b.appendChild(span2(flagsRow("flags", [
    flagToggle("terminal", !!o.terminal, v => { if (v) o.terminal = true; else delete o.terminal; ctx.refreshView(); }, { title: "final deliverable read outside the pipeline" }),
    flagToggle("optional", !!o.optional, v => { if (v) o.optional = true; else delete o.optional; ctx.refreshView(); }, { title: "may be absent on some runs" }),
  ])));
